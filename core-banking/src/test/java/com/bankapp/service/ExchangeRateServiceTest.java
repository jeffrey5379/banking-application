...
class ExchangeRateServiceTest {
    ...
    @Test
    void convert_eurUsdRoundTrip_returnsExactlyToStartingAmount() {

        BigDecimal eur = new BigDecimal("100000.00");

        BigDecimal usd = exchangeRateService.convert(eur, Currency.EUR, Currency.USD);
        BigDecimal backToEur = exchangeRateService.convert(usd, Currency.USD, Currency.EUR);

        assertThat(backToEur).isEqualByComparingTo(eur);
    }

    @Test
    void convert_sekEurRoundTrip_driftStaysBoundedAcrossManyRepetitions() {
        BigDecimal start = new BigDecimal("100.01");

        BigDecimal afterTwoRoundTrips = repeatSekEurRoundTrip(start, 2);
        BigDecimal afterTwentyRoundTrips = repeatSekEurRoundTrip(start, 20);

        assertThat(afterTwentyRoundTrips)
                .isEqualByComparingTo(afterTwoRoundTrips)
                .isCloseTo(start, within(new BigDecimal("0.01")));
    }

    private BigDecimal repeatSekEurRoundTrip(BigDecimal amount, int roundTrips) {
        BigDecimal result = amount;
        for (int i = 0; i < roundTrips; i++) {
            result = exchangeRateService.convert(result, Currency.SEK, Currency.EUR);
            result = exchangeRateService.convert(result, Currency.EUR, Currency.SEK);
        }
        return result;
    }
