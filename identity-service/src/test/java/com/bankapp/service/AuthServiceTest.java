...
class AuthServiceTest {
    ...
    void setUp() {
        ...
        aliceResponse = new UserResponse(ALICE_PUBLIC_ID, "alice", "alice@example.com");
    }

    // ── register ──────────────────────────────────────────────────────────

    @Test
    void register_validRequest_encodesPasswordCreatesUserAndIssuesOtpChallenge() {
        ...
        LoginChallengeResponse result = authService.register(new RegisterRequest("alice", "alice@example.com", "password123"));
        ...
        // No token is issued at registration time - only verifyOtp() does that, same as login.
    ...
    void verifyOtp_validCode_returnsToken() {
        ...
        when(userService.getUserByUsername("alice")).thenReturn(aliceResponse);
        when(jwtService.generateToken(aliceDetails, ALICE_PUBLIC_ID)).thenReturn("jwt.token.here");
